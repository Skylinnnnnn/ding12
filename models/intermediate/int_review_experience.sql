-- Reviews joined to delivery performance for customer-experience analysis.
-- Grain: one row per review record (from stg_order_reviews). An order can have
-- more than one review, so this is NOT one row per order. Downstream marts that
-- need order-level experience should aggregate accordingly.
with reviews as (
